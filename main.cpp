...
int main() {
    ...
    //added game timer because my pc can run the game really fast 
    ALLEGRO_TIMER* game_timer = al_create_timer(1.0 / 90.0);
    ...
    while (!done) {
        ...
        if (ev.type == ALLEGRO_EVENT_TIMER) {
            ...
            else if (ev.timer.source == spawn_timer) {
                for (int i = 0; i < 10; i++) {
                    bullet b;
                    b.set_bitmap(shared_bullet_bmp);
                    b.fire(rand() % (width - 16), 0);
        ...
        if (redraw && al_is_event_queue_empty(event_queue)) {
            redraw = false;
            bar_width = (countdown / 30.0) * width;
            ...
            for (int i = 0; i < (int)bullets.size(); i++) {
                if (bullets[i].get_status()) {
                    ...
                    if (my_cat.get_x() < bullets[i].get_x() + 16 && my_cat.get_x() + 64 > bullets[i].get_x() &&
                        my_cat.get_y() < bullets[i].get_y() + 16 && my_cat.get_y() + 64 > bullets[i].get_y()) {
                        score++;
